function optimize(env, demand::D) where {D <: Demand}
    ...
    for i in 1:action_space
        R[1, i], traceback_actions[1, i] = findmin([step_cost(env, demand, int2vec(x, env.N, env.M), int2vec(i, env.N, env.M), 1) for x in 1:action_space])
    end
    #recursion steps
    for t in 2:env.T-1
        for i in 1:action_space
            R[t, i], traceback_actions[t, i] = findmin([step_cost(env, demand, int2vec(x, env.N, env.M), int2vec(i, env.N, env.M), t) + R[t-1, x]
